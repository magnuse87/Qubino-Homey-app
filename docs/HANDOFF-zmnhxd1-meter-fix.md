...
Status as of 2026-09-22. Written for whoever (human or Claude Code) continues this work.
...
## 2. What has been changed (all uncommitted, on top of `origin/main` = 3e485e6 "Bump version to v4.1.6")
...
3. **Soak test.** Leave `meterPollingInterval` at 900 s (default) and let it run for days. Confirm the "restart
   the app every 5 minutes" Flow can be disabled/deleted. Watch for stuck values.
4. **Commit.** Only the files listed in `CLAUDE.md` gotcha 6 have real changes; the rest of `git status` is
   CRLF noise. Consider adding a `.gitattributes` (`* text=auto`) or setting `core.autocrlf` before committing so
   the diff stays reviewable. Suggested split: (a) polling fix + settings, (b) association fix + versioned flag,
   (c) version/changelog.
...
logged in via `homey login`. Repo at `C:\WORKSPACE\Homey\Qubino-Homey-app`, branch `main`, remote = upstream.
